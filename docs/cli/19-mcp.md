...
Antigravity 支持 [Model Context Protocol (MCP)](https://modelcontextprotocol.io)，这是一项开放标准，可让 AI agent 与编辑器安全地连接本地开发者工具、数据库、文件解析器以及外部远程 API。此集成让 AI 模型能够获取超出当前 workspace 范围的实时上下文与执行能力。

在本指南中，你将学习如何在 Antigravity 各产品中连接并配置 MCP server。你也可以直接跳转到 [Antigravity 2.0](/docs/mcp#antigravity-20)、[Antigravity IDE](/docs/mcp#antigravity-ide)、[Antigravity CLI](/docs/mcp#antigravity-cli) 或 [Antigravity SDK](/docs/mcp#antigravity-sdk) 的 MCP server 相关信息。
...
*   在编写 SQL 查询时，Antigravity 可以检查你实时的 Neon、Supabase 或 AlloyDB schema，以建议正确的表和列名称。
*   在调试部署失败时，Antigravity 可以直接从 Netlify 或 Heroku 拉取最近的构建日志。

### 添加自定义工具

借助 MCP，Antigravity 可以执行由你连接的 server 定义的特定安全操作：

*   为此 TODO 创建一个 Linear issue。
...
2.  选择 **Customizations**，然后查看 **Installed MCP Servers** 部分。
...
在 Antigravity IDE 中，管理 MCP server 最便捷的方式是通过内置的 MCP Store。在 MCP Store 中，你可以浏览、发现并安装受支持的 MCP server。你也可以通过更新 `mcp_config.json` 来安装自定义 server。
...
安装完成后，该 server 提供的资源与工具将自动在编辑器中可用。

连接未在商店中列出的自定义 MCP server：
...
该配置文件全局位于 `~/.gemini/config/mcp_config.json`（或位于你 workspace 本地的 `.agents/mcp_config.json`）。
...
在 prompt 面板中输入 `/mcp` 并按 `Enter`，即可打开交互式 **MCP Manager Overlay**。此面板允许你：

*   查看处于活动、断开或加载中状态的 server 的实时状态环。
*   手动重新加载 server 配置或检查实时连接日志。

### 全局与 Workspace Server 配置

与旧版设置不同，Antigravity CLI 将 MCP 定义拆分到专用的稀疏配置中：

*   **全局 server 设置：** 在 `~/.gemini/config/mcp_config.json` 中配置。
*   **Workspace 本地设置：** 在你当前项目下的 `.agents/mcp_config.json` 中配置。
...
在使用 [Antigravity SDK](/docs/sdk/overview) 构建的 Python 应用中，MCP server（`stdio`、`SSE` 或 `HTTP`）可以与内置工具和自定义 Python 函数一起，在统一的执行流水线中以编程方式连接。
...
*   **`env`**（object）：`stdio` server 进程的环境变量。
*   **`cwd`**（string）：`stdio` server 的工作目录。
*   **`headers`**（object）：远程 server 的自定义 HTTP header。
...
*   **`disabled`**（boolean）：临时禁用某个 server，而无需移除其配置。
*   **`disabledTools`**（string\[\]）：不向模型提供的工具名称。

## MCP 认证

已连接的 MCP server 可以使用内置 Google 凭据、自动 OAuth 流程或自定义 HTTP header，安全地向外部服务进行认证。

### Google 凭据

将 `authProviderType` 设置为 `"google_credentials"`，即可使用 Google Application Default Credentials (ADC)。
...
如果你手动提供了客户端凭据，请确保在你的 OAuth 提供方中将以下地址注册为重定向 URI：
...
2.  导航到 **Customizations** 标签页，然后点击 server 旁边的 **Authenticate** 按钮。
...
对 Model Context Protocol 工具与资源的访问由 Antigravity 的 [权限系统](/docs/permissions) 管理。默认情况下，未配置的 MCP 工具以 **Ask** 模式运行，执行前需要你的批准。你可以在策略配置中允许特定工具或整个 server：
...
MCP Store 为各类开发者平台、数据库与生产力服务提供直接集成：
